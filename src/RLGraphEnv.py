...
class RLGraphEnv:
    ...
    #     current_emb = self.trainer.get_embedding(self.current_node)

    #     if len(self.path) > 1:
    #         path_embs = [self.trainer.get_embedding(n) for n in self.path[:-1]]
    #         path_embs_tensor = torch.tensor(path_embs, dtype=torch.float32)
    #         pooled_path_emb = path_embs_tensor.mean(dim=0)  # or .sum(dim=0)
    #         combined = torch.cat([torch.tensor(current_emb, dtype=torch.float32), pooled_path_emb])
    #     else:
    #         combined = torch.cat([torch.tensor(current_emb, dtype=torch.float32),
    #                             torch.zeros_like(torch.tensor(current_emb, dtype=torch.float32))])
