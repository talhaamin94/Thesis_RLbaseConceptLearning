...
from utils.TransformLabels import TransformLabels
...
class RDFGraphConverter:
    ...
    def _assign_labels(self):
        ...
        if self.dataset_name == "mini":
            ...
            self._assign_labels_to_mutag()
        elif self.dataset_name in ["aifb"]:
    ...
    def _assign_labels_to_mutag(self):
        train_file = f"data/{self.dataset_name}/raw/trainingSet.tsv"
        test_file = f"data/{self.dataset_name}/raw/testSet.tsv"
        ...
        if target_node_type not in self.hetero_data.node_types:
            print(f"[ERROR] '{target_node_type}' not found in hetero_data.")
            return

        num_nodes = self.hetero_data[target_node_type].x.size(0)
        ...
        for dataset, df, mask in [("train", train_df, train_mask), ("test", test_df, test_mask)]:
            for _, row in df.iterrows():
                instance_uri = str(row["bond"]).strip()
                instance_name = instance_uri.split("#")[-1] if "#" in instance_uri else instance_uri.split("/")[-1]

                if (target_node_type, instance_name) in self.node_mapping:
                    node_index = self.node_mapping[(target_node_type, instance_name)]["node_index"]
                    labels[node_index] = int(row["label_mutagenic"])
                    mask[node_index] = True
                else:
                    print(f"[WARNING] Compound '{instance_name}' not found in node_mapping.")
